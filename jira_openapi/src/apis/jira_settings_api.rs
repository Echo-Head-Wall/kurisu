...
pub async fn get_advanced_settings(configuration: &configuration::Configuration, ) -> Result<Vec<models::ApplicationProperty>, Error<GetAdvancedSettingsError>> {
    ...
    let uri_str = format!("{}/rest/api/3/application-properties/advanced-settings", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ApplicationProperty&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ApplicationProperty&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_application_property(configuration: &configuration::Configuration, key: Option<&str>, permission_level: Option<&str>, key_filter: Option<&str>) -> Result<Vec<models::ApplicationProperty>, Error<GetApplicationPropertyError>> {
    ...
    let uri_str = format!("{}/rest/api/3/application-properties", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
...
pub async fn get_configuration(configuration: &configuration::Configuration, ) -> Result<models::Configuration, Error<GetConfigurationError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Configuration`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Configuration`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn set_application_property(configuration: &configuration::Configuration, id: &str, simple_application_property_bean: models::SimpleApplicationPropertyBean) -> Result<models::ApplicationProperty, Error<SetApplicationPropertyError>> {
    ...
    let uri_str = format!("{}/rest/api/3/application-properties/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
