...
pub async fn create_project_avatar(configuration: &configuration::Configuration, project_id_or_key: &str, body: Option<serde_json::Value>, x: Option<i32>, y: Option<i32>, size: Option<i32>) -> Result<models::Avatar, Error<CreateProjectAvatarError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/{projectIdOrKey}/avatar2", configuration.base_path, projectIdOrKey=crate::apis::urlencode(p_project_id_or_key));
...
pub async fn delete_project_avatar(configuration: &configuration::Configuration, project_id_or_key: &str, id: i64) -> Result<(), Error<DeleteProjectAvatarError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/{projectIdOrKey}/avatar/{id}", configuration.base_path, projectIdOrKey=crate::apis::urlencode(p_project_id_or_key), id=p_id);
...
pub async fn get_all_project_avatars(configuration: &configuration::Configuration, project_id_or_key: &str) -> Result<models::ProjectAvatars, Error<GetAllProjectAvatarsError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/{projectIdOrKey}/avatars", configuration.base_path, projectIdOrKey=crate::apis::urlencode(p_project_id_or_key));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ProjectAvatars`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ProjectAvatars`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn update_project_avatar(configuration: &configuration::Configuration, project_id_or_key: &str, avatar: models::Avatar) -> Result<serde_json::Value, Error<UpdateProjectAvatarError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/{projectIdOrKey}/avatar", configuration.base_path, projectIdOrKey=crate::apis::urlencode(p_project_id_or_key));
