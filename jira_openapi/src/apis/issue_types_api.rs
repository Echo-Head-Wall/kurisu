...
pub async fn create_issue_type(configuration: &configuration::Configuration, issue_type_create_bean: models::IssueTypeCreateBean) -> Result<models::IssueTypeDetails, Error<CreateIssueTypeError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IssueTypeDetails`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IssueTypeDetails`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn create_issue_type_avatar(configuration: &configuration::Configuration, id: &str, size: i32, body: Option<serde_json::Value>, x: Option<i32>, y: Option<i32>) -> Result<models::Avatar, Error<CreateIssueTypeAvatarError>> {
    ...
    let uri_str = format!("{}/rest/api/3/issuetype/{id}/avatar2", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
...
pub async fn delete_issue_type(configuration: &configuration::Configuration, id: &str, alternative_issue_type_id: Option<&str>) -> Result<(), Error<DeleteIssueTypeError>> {
    ...
    let uri_str = format!("{}/rest/api/3/issuetype/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
...
pub async fn get_alternative_issue_types(configuration: &configuration::Configuration, id: &str) -> Result<Vec<models::IssueTypeDetails>, Error<GetAlternativeIssueTypesError>> {
    ...
    let uri_str = format!("{}/rest/api/3/issuetype/{id}/alternatives", configuration.base_path, id=crate::apis::urlencode(p_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::IssueTypeDetails&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::IssueTypeDetails&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
