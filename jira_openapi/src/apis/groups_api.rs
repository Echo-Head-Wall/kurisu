...
pub async fn bulk_get_groups(configuration: &configuration::Configuration, start_at: Option<i64>, max_results: Option<i32>, group_id: Option<Vec<String>>, group_name: Option<Vec<String>>, access_type: Option<&str>, application_key: Option<&str>) -> Result<models::PageBeanGroupDetails, Error<BulkGetGroupsError>> {
    ...
    let p_access_type = access_type;
    let p_application_key = application_key;

    let uri_str = format!("{}/rest/api/3/group/bulk", configuration.base_path);
    ...
    if let Some(ref param_value) = p_access_type {
        req_builder = req_builder.query(&[("accessType", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PageBeanGroupDetails`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PageBeanGroupDetails`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn find_groups(configuration: &configuration::Configuration, account_id: Option<&str>, query: Option<&str>, exclude: Option<Vec<String>>, exclude_id: Option<Vec<String>>, max_results: Option<i32>, case_insensitive: Option<bool>, user_name: Option<&str>) -> Result<models::FoundGroups, Error<FindGroupsError>> {
    ...
    if let Some(ref param_value) = p_exclude_id {
        req_builder = match "multi" {
            "multi" => req_builder.query(&param_value.into_iter().map(|p| ("excludeId".to_owned(), p.to_string())).collect::<Vec<(std::string::String, std::string::String)>>()),
            _ => req_builder.query(&[("excludeId", &param_value.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string())]),
        };
    }
    if let Some(ref param_value) = p_max_results {
        req_builder = req_builder.query(&[("maxResults", &param_value.to_string())]);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FoundGroups`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FoundGroups`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_users_from_group(configuration: &configuration::Configuration, groupname: Option<&str>, group_id: Option<&str>, include_inactive_users: Option<bool>, start_at: Option<i64>, max_results: Option<i32>) -> Result<models::PageBeanUserDetails, Error<GetUsersFromGroupError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PageBeanUserDetails`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PageBeanUserDetails`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn remove_group(configuration: &configuration::Configuration, groupname: Option<&str>, group_id: Option<&str>, swap_group: Option<&str>, swap_group_id: Option<&str>) -> Result<(), Error<RemoveGroupError>> {
    ...
    let p_swap_group_id = swap_group_id;
    ...
    if let Some(ref param_value) = p_swap_group_id {
        req_builder = req_builder.query(&[("swapGroupId", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
