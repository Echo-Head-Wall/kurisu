...
pub async fn create_project_category(configuration: &configuration::Configuration, project_category: models::ProjectCategory) -> Result<models::ProjectCategory, Error<CreateProjectCategoryError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ProjectCategory`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ProjectCategory`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_all_project_categories(configuration: &configuration::Configuration, ) -> Result<Vec<models::ProjectCategory>, Error<GetAllProjectCategoriesError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ProjectCategory&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ProjectCategory&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_project_category_by_id(configuration: &configuration::Configuration, id: i64) -> Result<models::ProjectCategory, Error<GetProjectCategoryByIdError>> {
    ...
    let uri_str = format!("{}/rest/api/3/projectCategory/{id}", configuration.base_path, id=p_id);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
...
pub async fn update_project_category(configuration: &configuration::Configuration, id: i64, project_category: models::ProjectCategory) -> Result<models::UpdatedProjectCategory, Error<UpdateProjectCategoryError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::UpdatedProjectCategory`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::UpdatedProjectCategory`")))),
        }
    } else {
        let content = resp.text().await?;
