...
pub async fn delete_issue_link_type(configuration: &configuration::Configuration, issue_link_type_id: &str) -> Result<(), Error<DeleteIssueLinkTypeError>> {
    ...
    let uri_str = format!("{}/rest/api/3/issueLinkType/{issueLinkTypeId}", configuration.base_path, issueLinkTypeId=crate::apis::urlencode(p_issue_link_type_id));
...
pub async fn get_issue_link_types(configuration: &configuration::Configuration, ) -> Result<models::IssueLinkTypes, Error<GetIssueLinkTypesError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IssueLinkTypes`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IssueLinkTypes`")))),
        }
    } else {
        let content = resp.text().await?;
