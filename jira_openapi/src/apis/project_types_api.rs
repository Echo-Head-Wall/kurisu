...
pub async fn get_accessible_project_type_by_key(configuration: &configuration::Configuration, project_type_key: &str) -> Result<models::ProjectType, Error<GetAccessibleProjectTypeByKeyError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/type/{projectTypeKey}/accessible", configuration.base_path, projectTypeKey=crate::apis::urlencode(p_project_type_key));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ProjectType`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ProjectType`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_all_accessible_project_types(configuration: &configuration::Configuration, ) -> Result<Vec<models::ProjectType>, Error<GetAllAccessibleProjectTypesError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/type/accessible", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ProjectType&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ProjectType&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_project_type_by_key(configuration: &configuration::Configuration, project_type_key: &str) -> Result<models::ProjectType, Error<GetProjectTypeByKeyError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/type/{projectTypeKey}", configuration.base_path, projectTypeKey=crate::apis::urlencode(p_project_type_key));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
