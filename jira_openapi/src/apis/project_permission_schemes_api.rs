...
pub async fn assign_permission_scheme(configuration: &configuration::Configuration, project_key_or_id: &str, id_bean: models::IdBean, expand: Option<&str>) -> Result<models::PermissionScheme, Error<AssignPermissionSchemeError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/{projectKeyOrId}/permissionscheme", configuration.base_path, projectKeyOrId=crate::apis::urlencode(p_project_key_or_id));
...
pub async fn get_project_issue_security_scheme(configuration: &configuration::Configuration, project_key_or_id: &str) -> Result<models::SecurityScheme, Error<GetProjectIssueSecuritySchemeError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/{projectKeyOrId}/issuesecuritylevelscheme", configuration.base_path, projectKeyOrId=crate::apis::urlencode(p_project_key_or_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
...
pub async fn get_security_levels_for_project(configuration: &configuration::Configuration, project_key_or_id: &str) -> Result<models::ProjectIssueSecurityLevels, Error<GetSecurityLevelsForProjectError>> {
    ...
    let uri_str = format!("{}/rest/api/3/project/{projectKeyOrId}/securitylevel", configuration.base_path, projectKeyOrId=crate::apis::urlencode(p_project_key_or_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ProjectIssueSecurityLevels`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ProjectIssueSecurityLevels`")))),
        }
    } else {
        let content = resp.text().await?;
