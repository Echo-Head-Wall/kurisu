...
pub async fn get_issue_security_level(configuration: &configuration::Configuration, id: &str) -> Result<models::SecurityLevel, Error<GetIssueSecurityLevelError>> {
    ...
    let uri_str = format!("{}/rest/api/3/securitylevel/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SecurityLevel`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SecurityLevel`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_issue_security_level_members(configuration: &configuration::Configuration, issue_security_scheme_id: i64, start_at: Option<i64>, max_results: Option<i32>, issue_security_level_id: Option<Vec<String>>, expand: Option<&str>) -> Result<models::PageBeanIssueSecurityLevelMember, Error<GetIssueSecurityLevelMembersError>> {
    ...
    let uri_str = format!("{}/rest/api/3/issuesecurityschemes/{issueSecuritySchemeId}/members", configuration.base_path, issueSecuritySchemeId=p_issue_security_scheme_id);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PageBeanIssueSecurityLevelMember`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PageBeanIssueSecurityLevelMember`")))),
        }
    } else {
        let content = resp.text().await?;
