...
pub async fn add_attachment(configuration: &configuration::Configuration, issue_id_or_key: &str) -> Result<Vec<models::Attachment>, Error<AddAttachmentError>> {
    ...
    let uri_str = format!("{}/rest/api/3/issue/{issueIdOrKey}/attachments", configuration.base_path, issueIdOrKey=crate::apis::urlencode(p_issue_id_or_key));
...
pub async fn expand_attachment_for_humans(configuration: &configuration::Configuration, id: &str) -> Result<models::AttachmentArchiveMetadataReadable, Error<ExpandAttachmentForHumansError>> {
    ...
    let uri_str = format!("{}/rest/api/3/attachment/{id}/expand/human", configuration.base_path, id=crate::apis::urlencode(p_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AttachmentArchiveMetadataReadable`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AttachmentArchiveMetadataReadable`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn expand_attachment_for_machines(configuration: &configuration::Configuration, id: &str) -> Result<models::AttachmentArchiveImpl, Error<ExpandAttachmentForMachinesError>> {
    ...
    let uri_str = format!("{}/rest/api/3/attachment/{id}/expand/raw", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AttachmentArchiveImpl`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AttachmentArchiveImpl`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_attachment(configuration: &configuration::Configuration, id: &str) -> Result<models::AttachmentMetadata, Error<GetAttachmentError>> {
    ...
    let uri_str = format!("{}/rest/api/3/attachment/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
...
pub async fn get_attachment_content(configuration: &configuration::Configuration, id: &str, redirect: Option<bool>) -> Result<Vec<serde_json::Value>, Error<GetAttachmentContentError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_id = id;
    let p_redirect = redirect;

    let uri_str = format!("{}/rest/api/3/attachment/content/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    if let Some(ref param_value) = p_redirect {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetAttachmentContentError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn get_attachment_meta(configuration: &configuration::Configuration, ) -> Result<models::AttachmentSettings, Error<GetAttachmentMetaError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::AttachmentSettings`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::AttachmentSettings`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_attachment_thumbnail(configuration: &configuration::Configuration, id: &str, redirect: Option<bool>, fallback_to_default: Option<bool>, width: Option<i32>, height: Option<i32>) -> Result<Vec<serde_json::Value>, Error<GetAttachmentThumbnailError>> {
    ...
    let p_id = id;
    let p_redirect = redirect;
    let p_fallback_to_default = fallback_to_default;
    let p_width = width;
    let p_height = height;

    let uri_str = format!("{}/rest/api/3/attachment/thumbnail/{id}", configuration.base_path, id=crate::apis::urlencode(p_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    if let Some(ref param_value) = p_redirect {
        req_builder = req_builder.query(&[("redirect", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_fallback_to_default {
        req_builder = req_builder.query(&[("fallbackToDefault", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_width {
        req_builder = req_builder.query(&[("width", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_height {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetAttachmentThumbnailError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
