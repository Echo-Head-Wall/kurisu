...
pub async fn create_screen_scheme(configuration: &configuration::Configuration, screen_scheme_details: models::ScreenSchemeDetails) -> Result<models::ScreenSchemeId, Error<CreateScreenSchemeError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScreenSchemeId`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScreenSchemeId`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn delete_screen_scheme(configuration: &configuration::Configuration, screen_scheme_id: &str) -> Result<(), Error<DeleteScreenSchemeError>> {
    ...
    let uri_str = format!("{}/rest/api/3/screenscheme/{screenSchemeId}", configuration.base_path, screenSchemeId=crate::apis::urlencode(p_screen_scheme_id));
...
pub async fn get_screen_schemes(configuration: &configuration::Configuration, start_at: Option<i64>, max_results: Option<i32>, id: Option<Vec<i64>>, expand: Option<&str>, query_string: Option<&str>, order_by: Option<&str>) -> Result<models::PageBeanScreenScheme, Error<GetScreenSchemesError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PageBeanScreenScheme`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PageBeanScreenScheme`")))),
        }
    } else {
        let content = resp.text().await?;
